...
```mermaid
flowchart TB
  CLI["opagent CLI<br/>(pi InteractiveMode + OpAgent 扩展包)<br/>hermes/pi 风格对话框 · 流式 · markdown · 确认门"]
  PI["pi AgentSession (loop / tools / sessions / provider)<br/>provider: DeepSeek(默认) / OpenAI / Anthropic<br/>tools: read bash edit write grep find ls + ops 自定义"]
  SAFETY["opagent-safety 扩展<br/>tool_call / spawnHook<br/>路径 / 黑名单"]
  AUDIT["opagent-audit 扩展<br/>tool_call / tool_result<br/>哈希链"]
  TOOLS["ops 工具 (defineTool)<br/>inspect / monitor / security"]
  SKILLS["ops 技能 SKILL.md<br/>(Agent Skills)<br/>progressive disclosure"]
  DB[("bun:sqlite<br/>audit / FTS")]

  CLI -->|createAgentSession {extensions, customTools, skills}| PI
  PI --> SAFETY
  PI --> AUDIT
  PI --> TOOLS
  PI --> SKILLS
...
```mermaid
flowchart LR
  MASTER["主 agent (opagent)"]
  CHILD_B["子 agent<br/>(pi --mode rpc + OpAgent 扩展)<br/>@ server B"]
  CHILD_C["子 agent<br/>(pi --mode rpc + OpAgent 扩展)<br/>@ server C"]
  MASTER -->|RpcClient JSON-RPC| CHILD_B
  MASTER -->|RpcClient JSON-RPC| CHILD_C
```
